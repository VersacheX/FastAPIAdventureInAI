  ...
  const handleLoreLookup = async () => {
    ...
    try {
      const response = await axios.post(
        ...
        { lookup_prompt: prompt, story_preface: preface || undefined },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <div className="modal-card modal-card-wide">
        ...
        <div className="modal-body">
          ...
          <div className="form-group" style={{ border: '1px solid #333', borderRadius: '4px', padding: '0.5rem' }}>
            <button
              type="button"
            ...
            >
              <span>📚 Lore Helper {lookupPrompt && `(${lookupPrompt.length} chars)`}</span>
              <span style={{ fontSize: '0.85rem' }}>{showLoreHelper ? '▲' : '▼'}</span>
            </button>
            {showLoreHelper && (
              <div id="lore-helper-panel" style={{ marginTop: '0.75rem' }}>
                <input
                  ...
                  className="form-input"
                  style={{ width: '100%' }}
                  placeholder="e.g. Scooby Doo mystery settings, Midgar FF7 locations, Hollow Knight factions"
                  value={lookupPrompt}
                  onChange={(e) => setLookupPrompt(e.target.value)}
                  disabled={creating || loreLoading}
                />
                <small className="form-hint">
                  Provide franchise or thematic keywords. Generates structured descriptive tokens via AI.
                </small>
                ...
                {loreError && <div className="login-error" style={{ marginTop: '0.5rem' }}>{loreError}</div>}
                {loreDraft && !loreError && (
                  <div className="preview-box" style={{ marginTop: '0.75rem', maxHeight: '220px', overflowY: 'auto', border: '1px solid #444', padding: '0.5rem', width: '100%' }}>
                    <strong>Lore Draft Preview</strong>
                    <div style={{ fontSize: '0.8rem', marginTop: '0.25rem' }}>
                      {loreDraft.summary || 'No summary.'}
                    </div>
                    {loreDraft.draft_tokens && (
                      <pre style={{ whiteSpace: 'pre-wrap', fontSize: '0.7rem', marginTop: '0.5rem', width: '100%' }}>{loreDraft.draft_tokens}</pre>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="form-group">
            ...
            {loreDraft === null && lookupPrompt.trim() && !loreLoading && (
              <small className="form-hint" style={{ color: '#888' }}>
                Tip: Generate a lore draft above and insert it here to enrich the world.
              </small>
            )}
          </div>
