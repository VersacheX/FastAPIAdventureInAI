...
Usage:
 python tools/scan_site_dumps_fixed.py <dump_dir> [--output report.json] [--sample N]
...
def scan_folder(root_path, output_path=None, sample_count=3):
    ...
    for f in report['files']:
        ...
        hf = host_features.setdefault(
            h, {'count': 0, 'json_ld': 0, 'og': 0, 'script_heavy': 0, 'samples': []}
        )
        hf['count'] += 1
        if f['has_json_ld']:
            hf['json_ld'] += 1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Scan HTML dump directory and produce inventory report'
    )
    parser.add_argument('dump_dir', help='Path to directory containing HTML dump files')
    parser.add_argument(
        '--output', '-o', help='Path to write JSON report', default='site_dumps_report.json'
    )
    parser.add_argument(
        '--sample', '-s', help='Number of samples per host', type=int, default=3
    )
