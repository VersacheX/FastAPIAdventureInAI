...
async def perform_register(user: UserRegister, db: Session = Depends(get_db)):
    """
...
async def perform_login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    """
